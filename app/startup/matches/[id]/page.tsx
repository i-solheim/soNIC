...
import { ArrowLeft, Brain, AlertTriangle, Calendar } from "lucide-react";
...
function scoreColor(score: number) {
  if (score >= 85) return "bg-[var(--color-success)] text-white";
  if (score >= 70) return "bg-[var(--color-warning)] text-white";
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        >
          <div className="flex items-start justify-between gap-6 flex-wrap">
            ...
            <div
              className={cn(
                "w-20 h-20 rounded-full flex flex-col items-center justify-center flex-shrink-0 shadow-lg text-white",
                scoreColor(match.score)
              )}
            >
              <span className="text-xl font-black leading-none">{match.score}%</span>
              <span className="text-[10px] font-medium opacity-80 mt-0.5">match</span>
            </div>
          </div>
        </motion.div>
        ...
        >
          ...
          <CapabilityComparison
            ...
            startupHasLabel={t("match.detail.startupHas")}
            orgNeedsLabel={t("match.detail.orgNeeds")}
          />
        </motion.div>
        ...
        >
          ...
          <button className="btn-outline flex-1">{t("matches.express")}</button>
        </motion.div>
      </div>
    </DashboardLayout>
  );
